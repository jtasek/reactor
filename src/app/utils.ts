...
export function isEllipseInBox(ellipse: Ellipse, box: Box): boolean {
  ...
  const leftEllipse = ellipse.center.x - ellipse.radiusX;
  const rightEllipse = ellipse.center.x + ellipse.radiusX;
  const topEllipse = ellipse.center.y - ellipse.radiusY;
  const bottomEllipse = ellipse.center.y + ellipse.radiusY;
