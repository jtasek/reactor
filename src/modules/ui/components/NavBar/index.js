...
const NavBarItem = connect(
  ...
  ({ /*isDragging, connectDragSource,*/ shape, handler }) => (
    ...
    <li className={styles.active}>
      <a onClick={() => handler({ id: shape.id })}>{shape.name}</a>
    </li>
  )
)
...
const NavBarList = ({ name, items, handler }) => (
  <li>
    ...
    <ul>
      {items.map(item => (
        <NavBarItem key={item} name={name} item={item} handler={handler} />
      ))}
    </ul>
  </li>
)
...
export default connect(
  {
    ...
    shapeSelectionChanged: sequences`ui.shapeSelectionChanged`
  },
  NavBar
)
