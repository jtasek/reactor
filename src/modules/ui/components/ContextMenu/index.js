...
function renderButtons(tools, onClickHandler) {
    ...
    return Object.keys(tools).map(
        (name, index) => {
            let angle = anglegen.next().value
            let tool = tools[name]
            let style = getInlineStyle(15, angle, tool.active)

            return <ContextMenuButton key={index} tool={tool} inlineStyles={style} onClickHandler={() => onClickHandler({ name: name })} />
        }
    )
}
...
const ContextMenuButton = ({ tool, inlineStyles, onClickHandler }) => (
    <li className={styles.contextMenuButton} style={inlineStyles}>
        <a href="#" onClick={(e) => { e.preventDefault(); onClickHandler() }} title={tool.description}>
            <Icon {...tool.icon} />
        </a>
    </li>
)
...
const ContextMenu = ({ visible, position, tools, toolActivated }) => (
    <ul className={styles.contextMenu} style={!visible ? { display: 'none' } : { display: 'block', position: 'absolute', top: position.y, left: position.x }}>
